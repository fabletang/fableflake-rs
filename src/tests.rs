...
use crate::{
    ...
    error::*,
    fableflake::{BIT_LEN_SEQUENCE, BIT_LEN_TIME, Fableflake, decompose, to_fableflake_time},
};

#[test]
fn test_next_id() -> Result<(), BoxDynError> {
    let sf = Fableflake::new()?;
...
fn test_once() -> Result<(), BoxDynError> {
    let now = Utc::now();
    let sf = Fableflake::builder().start_time(now).finalize()?;
...
fn test_run_for_10s() -> Result<(), BoxDynError> {
    let now = Utc::now();
    let start_time = to_fableflake_time(now)?;
    let sf = Fableflake::builder().start_time(now).finalize()?;
    ...
    let initial = to_fableflake_time(Utc::now())?;
    ...
    while current - initial < 1000 {
        ...
        current = to_fableflake_time(Utc::now())?;
...
fn test_generate_10_ids() -> Result<(), BoxDynError> {
    let sf = Fableflake::builder().machine_id(&|| Ok(42)).finalize()?;
    let mut ids = vec![];
    for _ in 0..10 {
        let id = sf.next_id()?;
        assert!(
...
#[test]
fn test_default_epoch_is_2023_07_21_utc_plus_8() -> Result<(), BoxDynError> {
    let sf = Fableflake::builder()
        .machine_id(&|| Ok(1))
        .finalize()
        .expect("failed to build with deterministic machine id");

    assert_eq!(sf.0.start_time, 0);
    Ok(())
}

#[test]
fn test_to_fableflake_time_starts_from_epoch() -> Result<(), BoxDynError> {
    let epoch = FixedOffset::east_opt(8 * 3600)
        .unwrap()
        .with_ymd_and_hms(2023, 7, 21, 0, 0, 0)
        .single()
        .unwrap()
        .with_timezone(&Utc);

    assert_eq!(to_fableflake_time(epoch)?, 0);
    Ok(())
}

#[test]
fn test_monotonic_ids_single_thread() -> Result<(), BoxDynError> {
    let sf = Fableflake::builder().machine_id(&|| Ok(7)).finalize()?;
    let mut last = sf.next_id()?;
    for _ in 0..10_000 {
        let current = sf.next_id()?;
        assert!(current > last, "id must be strictly monotonic");
        last = current;
    }
    Ok(())
}
