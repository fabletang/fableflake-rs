...
use crate::{
    ...
    fableflake::{BIT_LEN_SEQUENCE, Fableflake, Internals, SharedFableflake, to_fableflake_time},
};

/// A builder to build a [`Fableflake`] generator.
///
/// [`Fableflake`]: struct.Fableflake.html
...
impl<'a> Builder<'a> {
    /// Construct a new builder to call methods on for the [`Fableflake`] construction.
    ///
    /// [`Fableflake`]: struct.Fableflake.html
    #[must_use]
    pub fn new() -> Self {
    ...
    /// Finalize the builder to create a Fableflake.
    ///
    /// # Errors
    ///
    ...
    pub fn finalize(self) -> Result<Fableflake, Error> {
        ...
        let start_time = if let Some(start_time) = self.start_time {
            ...
            to_fableflake_time(start_time)?
        } else {
            // Default start time is exactly the custom epoch.
            0
        };
        ...
        let shared = Arc::new(SharedFableflake {
        ...
        Ok(Fableflake::new_inner(shared))
    }
}
...
#[cfg(feature = "pnet")]
fn private_ipv6() -> Option<Ipv6Addr> {
    pnet_datalink::interfaces()
        .iter()
        .filter(|interface| interface.is_up() && !interface.is_loopback())
        .flat_map(|interface| interface.ips.iter())
...
pub(crate) fn lower_16_bit_private_ip() -> Result<u16, Error> {
    if let Some(ipv4) = private_ipv4() {
        ...
        return Ok((u16::from(octets[2]) << 8) + u16::from(octets[3]));
    }

    if let Some(ipv6) = private_ipv6() {
        let octets = ipv6.octets();
        return Ok((u16::from(octets[14]) << 8) + u16::from(octets[15]));
    }

    Err(Error::NoPrivateIpAddress)
}
